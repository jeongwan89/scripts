...
# 참고로 if절 내에서 변수를 생성하려면,
# if-yes의 변수 생성 구조와
# if-else의 변수 생성 구조가 일치해야 된다. 이유는 구절 밖에서 변수 생성이 되지 않으므로, 스크립의 consistence를 유지하려면 그렇다.
